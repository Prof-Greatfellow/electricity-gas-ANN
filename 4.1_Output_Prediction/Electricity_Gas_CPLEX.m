tic;
fid = fopen('training_test_set.csv', 'w');
...
nodal_pressure_squared=sdpvar(No_of_nodes,1,'full');
...
CONT1= node_param(:,4).^2<=nodal_pressure_squared<=node_param(:,3).^2;
...
for i=1:No_of_compressors
    CONT5=[CONT5,(compressor_param(i,4))^2 * nodal_pressure_squared(compressor_param(i,2))<= ...
        nodal_pressure_squared(compressor_param(i,3))<=(compressor_param(i,5))^2 * nodal_pressure_squared(compressor_param(i,2))];
end
...
%% Gas Flow Equation
W=sdpvar(No_of_pipelines,1,'full');
CONT9=[];
for i=1:No_of_pipelines
    CONT9=[CONT9,-Big_M*direction_flow(i)<=nodal_pressure_squared(pipeline_param(i,2))-nodal_pressure_squared(pipeline_param(i,3)),...
        nodal_pressure_squared(pipeline_param(i,2))-nodal_pressure_squared(pipeline_param(i,3))<=Big_M*(1-direction_flow(i))];   % OK
    CONT9=[CONT9,-Big_M*direction_flow(i)<=pipeline_flow(i),pipeline_flow(i)<=Big_M*(1-direction_flow(i))];  %OK
    CONT9=[CONT9,-Big_M*direction_flow(i)+nodal_pressure_squared(pipeline_param(i,2))-nodal_pressure_squared(pipeline_param(i,3))<=W(i), ...
        W(i)<=Big_M*direction_flow(i)+nodal_pressure_squared(pipeline_param(i,2))-nodal_pressure_squared(pipeline_param(i,3))]; %OK
    CONT9=[CONT9,-Big_M*(1-direction_flow(i))-nodal_pressure_squared(pipeline_param(i,2))+nodal_pressure_squared(pipeline_param(i,3))<=W(i),...
        W(i)<=Big_M*(1-direction_flow(i))-nodal_pressure_squared(pipeline_param(i,2))+nodal_pressure_squared(pipeline_param(i,3))]; %OK
end

CONT10=[];
K=pipeline_param(:,4).^2;
for i=1:No_of_pipelines
    CONT10=[CONT10,norm([2*pipeline_flow(i),K(i)-W(i)])<=K(i)+W(i)];  % OK
end
coeff=0.1;
rng(1);
toc;
...
direction_flow=zeros(No_of_pipelines,1);
...
CONT14= power_injection==0;
%% Set objective and solve
objective=sum(source_param(:,5).*source_generation)+sum(gen_param(:,4).*power_generation*1e6);
penalty_factor=0.001*ones(No_of_pipelines,1);
penalty=sum(penalty_factor.*(K+W));
...
for iteration=1:Max_iteration
    sol=optimize(Constraints,objective+penalty,ops); 
    if sol.problem~=0
        break_indicator=0;
        break
    end
    ...
    if max(error)>epsilon
        for i=1:No_of_compressors
            if error(i)>epsilon
                ...
                current_p_out=value(nodal_pressure_squared(compressor_param(i,3)));
                current_p_in=value(nodal_pressure_squared(compressor_param(i,2)));
                ...
                temp=gradient_comp(alpha_list(i),beta_list(i),B_list(i),current_flow, ...
                ...
                Constraints=[Constraints,compressor_consumption(i)>= ...
                    ...
                    gradient_out*(nodal_pressure_squared(compressor_param(i,3))-current_p_out)+ ...
                    gradient_in*(nodal_pressure_squared(compressor_param(i,2))-current_p_in)];
